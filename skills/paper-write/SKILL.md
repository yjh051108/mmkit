---
name: paper-write
description: "Draft a paper section by section from an outline, in LaTeX (PDF mode) or Markdown (docx mode). Use when user says \"write paper\", \"draft LaTeX\", \"draft Word paper\", or wants to generate paper content from a paper plan."
...
## docx mode conventions

When `params.output_format == 'docx'`, read `references/docx-cn-engine.md` in this skill for the Markdown conventions the downstream converter expects (headings, math, figures, tables, references). In docx mode: write `paper/main.md` only, never `.tex`, never LaTeX commands.
...
echo ""
echo "=== TikZ diagrams ==="
# TikZ 图由 paper-figure 生成为 figures/tikz_diagrams.tex → 编译成 figures/tikz_diagrams.pdf
# （历史命名可能是 tikz_architecture_examples.tex，一并兼容）。
# TikZ 的 PDF 已经由 paper-figure 写进 latex_includes.tex，按 latex_includes.tex 嵌入即可。
...
- **TikZ diagrams must be embedded** into corresponding sections — every `tikz_*.pdf` referenced in `latex_includes.tex` must appear in some section (paper-figure already added include blocks for them)
