---
name: format-profile
description: "根据用户的文字格式要求或上传的格式说明文档，生成 docx-format-check 用的样式 profile JSON。Claude 自主理解中文字号术语、字体、行距、缩进等参数。"
...
读取用户的文字格式要求，生成 `_text_profile.json` 供 docx-format-check 使用。
...
- `_text_profile.json` — 派生的样式 profile，docx-format-check 步骤会读取并应用
...
4. **JSON 必须合法**，否则 docx-format-check 会回退到默认 profile
