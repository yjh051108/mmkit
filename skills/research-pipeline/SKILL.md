...
- **ARXIV_DOWNLOAD = false** — When `true`, `/literature-review` downloads the top relevant arXiv PDFs during literature survey. When `false` (default), only fetches metadata via arXiv API. Passed through to `/idea-discovery` → `/literature-review`.
...
This internally runs: `/literature-review` → `/idea-creator` → `/novelty-check` → `/research-review`
