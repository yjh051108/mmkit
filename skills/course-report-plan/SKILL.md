...
if [ -n "$TEMPLATE_DOCX" ]; then
    ...
    echo "（docx-format-check 步骤会自动提取该模板格式并应用到导出 Word）"
fi
...
**写完上面的图表清单后，在产出文档（PAPER_PLAN.md / PROBLEM_ANALYSIS.md / 等）的**最后**追加一个机器可读的清单区块。下游 paper-figure / paper-figure / 写作 SKILL / workflow_engine.py 都按此清单对账。少一张就触发 AUTO-RECOVER。**
...
**DrawIO 流程/架构图（paper-figure 产出 .drawio + .png/.pdf）：**
- fig_arch
- fig_flow_xxx
