...
description: "文献综述撰写/论文检索与相关工作分析。统一入口：中文综述（默认）、英文检索分析（research 模式）、通信领域专项（comm 模式）。检索学术文献、验证真实性、按主题分类、撰写综合性文献综述。Use when user says \"文献综述\", \"literature review\", \"综述\", \"find papers\", \"related work\"."
...
| 模式 | 触发 | 执行 |
|---|---|---|
| `zh`（默认） | 中文综述请求 | 下方完整工作流程（中英检索、主题聚类、综合综述） |
| `research` | "find papers" / "related work" / 英文研究场景 | 读 `references/research-mode.md`，按其流程检索分析（BibTeX 获取、单篇精读、相关工作梳理） |
| `comm` | 通信领域（wireless/networking/satellite/5G/6G 等） | 读 `references/comm-domain.md`，按其知识库优先检索流程执行 |

`comm` 模式默认输出 `LITERATURE_REVIEW.md`（若任务要求）；`research` 模式按 `references/research-mode.md` 约定输出。
