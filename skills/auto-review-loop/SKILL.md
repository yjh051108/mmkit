...
description: Autonomous multi-round research review loop using any OpenAI-compatible reviewer (OpenAI / DeepSeek / MiniMax / Kimi / GLM / SiliconFlow / Qwen / Yi, configured via env vars). Repeatedly reviews via external reviewer script, implements fixes, and re-reviews until positive assessment or max rounds reached. Use when user says "auto review loop", "review until it passes", "llm review", "minimax review", or wants autonomous iterative improvement.
...
The external reviewer is invoked via `reviewer_client.py` (path injected through the `REVIEWER_SCRIPT` environment variable). The reviewer model is configured with three environment variables: `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `REVIEWER_MODEL_ID`.

The skill works with **any OpenAI-compatible API** — no code change needed, just point the three variables at your provider:
...
💡 **Why vary the reviewer model?** A different provider gives an independent review perspective — cross-model review improves objectivity. For example, using MiniMax as reviewer means configuring Base URL `https://api.minimax.chat/v1` and Model ID `MiniMax-M2.5`. The loop logic is identical regardless of provider.
...
3. **Final compile + quality gate** (gated by output mode, see preamble):
   ...
   - **Mode = `AUTO_REVIEW_DOCX_MODE`**: skip the LaTeX block entirely. Run only:
     ...
     ```
     Do **not** call xelatex/pdflatex. The downstream `docx-format-check` step converts `NARRATIVE_REPORT.md` to `NARRATIVE_REPORT.docx`.
