...
在导出 Word 之前对 Markdown 做最后一道格式自检，专注于**会让生成的 .docx 走样的格式问题**，并在能直接修复时立即修复（无需用户确认）。
...
- 图片说明（alt text）是否带「图 X：」前缀 — 转换引擎会把 alt 当图题
...
- 如果 Step 7 中「仍需人工处理」非空且影响 docx 可读性 → 写入报告但不退出（`exit 0`），让导出步骤继续，避免误伤
- 如果代码块标记不平衡（无法决定补哪头）→ 报告中标红，但 `exit 0`（让用户在最终 docx 里看到明显异常再回头修）
- **本步骤不强制阻塞工作流**，本自检步骤永远会跑
