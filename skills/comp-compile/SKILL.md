---
name: comp-compile
description: "数学建模竞赛论文编译与合规检查（中英文通用）。编译 PDF 并检查页数、匿名、格式等竞赛要求；自动检测 xelatex/pdflatex 引擎。Use when user says \"编译竞赛论文\", \"compile competition paper\"."
argument-hint: [paper-directory]
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob
---

# Competition Paper Compile & Compliance

Compile and validate: **$ARGUMENTS**

## Constants

- **ENGINE** — Auto-detect: `grep -q 'ctex\|xelatex\|xeCJK\|fontspec' paper/main.tex` → `xelatex`; otherwise `pdflatex`.
...
```bash
# Detect engine from paper source
if [ -f paper/main.tex ] && grep -q 'ctex\|xelatex\|xeCJK\|fontspec' paper/main.tex 2>/dev/null; then
    ENGINE=xelatex
else
    ENGINE=pdflatex
fi
...
if ! which "$ENGINE" 2>/dev/null; then
    echo "$ENGINE not found, attempting install..."
...
which "$ENGINE" && which bibtex && echo "ready" || echo "$ENGINE/bibtex not found"
...
**⛔ 修复后必须重新编译验证：** `$ENGINE → bibtex → $ENGINE → $ENGINE`，然后重新运行 compile_check.sh 确认无 FAIL。
...
# 10. TikZ 几何/算法/架构图
# TikZ 由 paper-figure 编译成 figures/tikz_diagrams.pdf, 通过 \includegraphics 嵌入
...
if grep -q 'tableofcontents' paper/main.tex 2>/dev/null; then
    [ -s paper/main.toc ] && echo "✅ TOC generated" || {
        ...
        cd paper/
        $ENGINE -interaction=nonstopmode main.tex > /dev/null 2>&1
        $ENGINE -interaction=nonstopmode main.tex > /dev/null 2>&1
        cd ..
