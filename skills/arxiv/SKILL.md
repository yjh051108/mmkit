...
- **FETCH_SCRIPT** - `tools/arxiv_fetch.py` relative to the mmkit install, or the same path relative to the current project. Fall back to inline Python if not found.
...
```text
/literature-review "topic"     - multi-source review: Zotero + Obsidian + local PDFs + web
...
- If the arXiv API is unreachable, report the error clearly and suggest using `/literature-review` with `- sources: web` as a fallback
