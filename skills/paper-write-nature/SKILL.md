...
description: "Draft a Nature-style paper with hourglass structure, reader-first logic, and publication-quality English — in LaTeX (PDF mode) or Markdown (docx mode). Use when user says 'Nature paper', 'Nature writing', 'SCI writing', or needs high-impact journal manuscript drafting."
...
When `params.output_format == 'docx'`, read `references/docx-conventions.md` in this skill for the Markdown conventions (headings / abstract / math / figures / tables / citations). In docx mode: write `paper/main.md` only, never `.tex`.
