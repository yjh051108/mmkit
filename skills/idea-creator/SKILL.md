...
Given a broad research direction from the user, systematically generate, validate, and rank concrete research ideas. This skill composes with the literature-review, novelty-check, and research-review steps to form a complete idea discovery pipeline.
...
0. **Read previous step outputs**: Check if `literature_review.md` or `references.bib` exist in the project root (produced by the `literature-review` step). If found, read them first to build upon the existing literature survey rather than duplicating the work.
...
Use the external LLM via `reviewer_client.py` for divergent thinking. **If the reviewer script fails (e.g. API key not configured)**, perform the brainstorming yourself — generate 8-12 ideas using the landscape map from Phase 1 as context, then proceed directly to Phase 3.

When the reviewer script is available:
...
1. **Novelty check**: Perform a thorough novelty verification (multi-source search + external reviewer cross-verification) for each idea

2. **Critical review**: Use the external reviewer via `reviewer_client.py` (same thread):
   ```bash
   cat << 'REVIEW_EOF' > _review_prompt.txt
...
2. **Deploy in parallel**: Launch pilots on different GPUs simultaneously using Bash + screen/tmux:
   ```
...
3. **Collect results**: Monitor experiment progress. If any pilot exceeds PILOT_TIMEOUT_HOURS, kill it and collect partial results. Once all pilots complete (or timeout), compare:
...
- [ ] If confirmed, run auto-review-loop for full iteration
```

## Key Rules

- **⛔ File writing strategy (prevent both failure modes):**
  - IDEA_REPORT.md is typically 200+ lines (with ranking rationale) → **Write the first section with Write tool (ensures file exists), then `cat << 'EOF' >> IDEA_REPORT.md` to append remaining sections**
  - If content is short (<150 lines) → just use Write directly
  - **NEVER end_turn without writing the report** — even if only 1-2 ideas survive filtering, write the report
...
After this skill produces the ranked report, the pipeline continues with:
```
