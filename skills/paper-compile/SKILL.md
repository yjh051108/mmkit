---
name: paper-compile
description: "Compile a LaTeX paper to PDF (English or Chinese). Auto-detects xelatex/pdflatex. Use when user says \"编译中文论文\", \"编译论文\", \"compile Chinese paper\", \"compile paper\", \"中文PDF\", or wants to compile an academic paper."
...
Compile a LaTeX paper: **$ARGUMENTS**

## Constants

- **ENGINE** — Auto-detect: `grep -q 'ctex\|xelatex\|xeCJK\|fontspec' paper/main.tex` → `xelatex` (required for Chinese); otherwise `pdflatex`.
...
if total == 0:
    ...
    print(f'✅ {total - miss}/{total} key values found in paper')
" 2>/dev/null
    [ $? -ne 0 ] && GATE_FAIL=$((GATE_FAIL+1))
fi
