...
把用户上传的 `.docx` / `.dotx` 模板转换成 `docx_template_fill.py` 能用的「占位映射 JSON」，并通过「生成→试填→分析→修复」循环确保映射正确，让 docx-format-check 用上的 map 是已验证的。
...
4. **最多 3 轮自检** — 第 3 轮还失败就如实记录，让后续 docx-format-check 退到正则启发式
...
修完 map 后回到 Step 4 的合法性自检 → Step 5 重试。**最多循环 3 次**，第 3 次仍失败也保存当前 map 让 docx-format-check 自己处理（fill_template 内部有 fallback）。
...
保留 `_template_test_filled.docx`（用户可下载查看预览），让后续 docx-format-check 步骤跑正式版。
