...
```
/literature-review → /novelty-check → [structure design] → [draft] → /research-review → [revise] → GRANT_PROPOSAL.md
...
- **REVIEWER_MODEL = `外部评审模型`** — Model used via external reviewer for proposal review. Configured in reviewer_client.py settings.
...
```json
{
  ...
  "language": "Japanese",
  "reviewer_thread_file": "_reviewer_thread.json",
  "gap_statement": "...",
  "aims_count": 3,
  "status": "in_progress",
...
- No literature survey → will invoke `/literature-review` inline in Phase 1
...
Invoke `/literature-review` to ground the proposal in real literature, then search for competing funded projects:
...
**What this does:**
- Reuse existing surveys if `/literature-review` was already run and notes exist
- Otherwise invoke `/literature-review` for multi-source literature search (arXiv, Scholar, Zotero, local PDFs)
...
**What this does:**
- The external reviewer acts as a grant review panelist (not a paper reviewer)
...
- Timeline: [summary]
- Reviewer feedback: [key points from external reviewer]
...
**State**: Write `GRANT_STATE.json` with `phase: 2`, aims summary.
...
**What this does:**
- The external reviewer acts as a grant review panelist
...
> ⚠️ **Reviewer fallback**: If the reviewer script fails (e.g. API key not configured), skip external review. Note "External review skipped — reviewer not configured. Consider running `/auto-review-loop` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.

If `/research-review` is invoked (preferred), it handles the reviewer call internally. If calling the reviewer directly (e.g., to maintain conversation context from Phase 2):
...
Implement CRITICAL and MAJOR fixes. If MAX_REVIEW_ROUNDS > 1, re-submit for another round via `reviewer_client.py` (multi-turn is automatic with `--thread-file`).
...
- Timeline: [N] years
- Review score: [summary from external reviewer]
...
- For long content (>150 lines): use **Write** for the first section (ensures file exists on disk), then append remaining sections with `cat << 'EOF' >> grant-proposal/GRANT_PROPOSAL.md`
- **NEVER `end_turn` without producing `grant-proposal/GRANT_PROPOSAL.md`** — even if upstream steps had issues, write what you have

⛔ **MUST run output verification before ending**:
```bash
PASS=true
[ -f grant-proposal/GRANT_PROPOSAL.md ] && SZ=$(wc -c < grant-proposal/GRANT_PROPOSAL.md) || SZ=0
if [ "$SZ" -ge 500 ]; then
    echo "✅ grant-proposal/GRANT_PROPOSAL.md ($SZ bytes)"
else
    echo "❌ grant-proposal/GRANT_PROPOSAL.md missing or too small ($SZ bytes) — write it NOW before ending"
    PASS=false
fi
...
| `sources` | all | Literature sources | → `/literature-review` |
| `arxiv download` | false | Download arXiv PDFs | → `/literature-review` |
| `reviewer model` | 外部评审模型 | External review model | → reviewer_client.py |
...
|-----------|:-----:|---------|
| `/literature-review` | 1 | Literature survey (if not already done) |
