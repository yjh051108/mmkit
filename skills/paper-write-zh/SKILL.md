---
name: paper-write-zh
description: "Draft a Chinese academic paper in LaTeX (XeLaTeX + ctex, PDF mode) or Markdown (docx mode). Use when user says \"写中文论文\", \"中文LaTeX\", \"中文Word论文\", \"Chinese paper writing\", or wants to generate Chinese paper content."
argument-hint: [topic]
...
## docx 模式（output_format = docx）

当 `params.output_format == 'docx'`：只产出 `paper/main.md` 一个文件（≥ 5KB），**禁止生成 .tex、禁止残留 LaTeX 命令**。Markdown 规范（标题层级/摘要/公式/图表/引用）见本 skill 的 `references/docx-mode-zh.md`。后续由 docx-cn-engine 转 .docx。
...
est_pages=$((total / 900))
echo "总字符: $total, 估算页数: ~$est_pages, 目标: ≥ ${MAX_PAGES:-30}"
