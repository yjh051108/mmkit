...
if [ -n "$PLAN_FILE" ]; then
  ...
  TOTAL_EXPECTED=$(echo "$EXPECTED_FIGS" | wc -l)
  MISSING_FIGS=""
  ...
  if [ "$MISSING_COUNT" -gt 0 ]; then
    echo "❌ FIGURE_MANIFEST 对账失败: 规划 $TOTAL_EXPECTED 张, 缺失 $MISSING_COUNT 张:"
    for m in $MISSING_FIGS; do echo "    - $m"; done
    echo "⛔ 必须把这些图全部产出才能结束 paper-figure 步骤"
    PASS=false
  else
    echo "✅ FIGURE_MANIFEST 全部产出: $TOTAL_EXPECTED 张"
  fi
else
...
if [ -z "$PLAN_FILE" ]; then
  ...
  EXPECTED_FIGS=$(echo "$MANIFEST" | grep -oE '^[[:space:]]*-[[:space:]]+(fig_[a-zA-Z0-9_]+|tikz_[a-zA-Z0-9_]+)' | sed 's/^[[:space:]]*-[[:space:]]*//')
  echo "  规划清单："
  echo "$EXPECTED_FIGS" | sed 's/^/    /'

  # 逐条检查产物是否存在 (任意一种格式都算: .png / .pdf / .drawio)
  MISSING_FIGS=""
  for name in $EXPECTED_FIGS; do
  ...
  TOTAL_EXPECTED=$(echo "$EXPECTED_FIGS" | wc -l)
  echo ""
  echo "  规划: $TOTAL_EXPECTED 张, 已产出: $((TOTAL_EXPECTED - MISSING_COUNT)) 张, 缺失: $MISSING_COUNT 张"
  if [ "$MISSING_COUNT" -gt 0 ]; then
    echo "  ❌ 缺失的图:"
    for m in $MISSING_FIGS; do echo "    - $m"; done
    echo ""
    echo "  ⛔ 必须把上面所有缺失的图都生成出来才能结束本步骤. 不允许跳过任何一张."
  else
    echo "  ✅ 全部产出"
  fi
fi
...
| 状态 | 行动 |
|---|---|
| FIGURE_MANIFEST 对账显示 **MISSING_COUNT > 0** | ⛔⛔⛔ **逐张补齐**：每个缺失的 `fig_xxx` 必须按其在规划文档里的描述去生成。`fig_flow_q*` / `fig_roadmap` / `fig_pipeline` 等 drawio 类的 → 调 paper-figure；`tikz_*` → 调 TikZ 子流程；其余数据图 → Step 3 写 `gen_fig_xxx.py` 执行画图 |
...
3. 如果 3 次重试全部失败 → 记录到 `_gptimg_failed.txt` → 由 paper-figure 步骤自行选择最合适的替代方案（DrawIO 或 TikZ，根据图的内容自主判断）
...
**7. Record failures for DrawIO fallback (persist to file for paper-figure step).**
...
if grep -qi 'drawio\|DrawIO\|架构图\|技术路线\|roadmap\|framework\|流程图' PAPER_PLAN.md TOPIC_PLAN.md PROBLEM_ANALYSIS.md 2>/dev/null; then
    # DrawIO/TikZ 检查已移至 paper-figure 步骤，此处跳过
    DRAWIO_COUNT=$(ls figures/*.drawio 2>/dev/null | wc -l)
    [ "$DRAWIO_COUNT" -gt 0 ] && echo "  (DrawIO: $DRAWIO_COUNT files — will be validated by paper-figure step)" || echo "  (no DrawIO yet — will be generated by paper-figure step)"
fi
...
# 4.5 TikZ/DrawIO — handled by paper-figure step, skip here
echo "  (TikZ/DrawIO diagrams will be generated and validated by the next step: paper-figure)"
