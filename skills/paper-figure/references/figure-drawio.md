...
if [ -n "$PLAN_FILE" ]; then
    ...
    # 只筛 drawio + tikz (drawio 类: fig_roadmap/flow_/pipeline/index_/gantt/network/framework/model_decision)
    EXPECTED_DRAWIO=$(sed -n "${START},${END}p" "$PLAN_FILE" | grep -oE '^[[:space:]]*-[[:space:]]+fig_(roadmap|flow_[a-zA-Z0-9_]+|pipeline[a-zA-Z0-9_]*|index_[a-zA-Z0-9_]+|gantt[a-zA-Z0-9_]*|network[a-zA-Z0-9_]*|framework[a-zA-Z0-9_]*|model_decision)' | sed 's/^[[:space:]]*-[[:space:]]*//')
    EXPECTED_TIKZ=$(sed -n "${START},${END}p" "$PLAN_FILE" | grep -oE '^[[:space:]]*-[[:space:]]+tikz_[a-zA-Z0-9_]+' | sed 's/^[[:space:]]*-[[:space:]]*//')
    drawio_missing=0
...
ALL COMPLETE — paper-figure step finished successfully
```
