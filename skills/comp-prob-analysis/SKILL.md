...
# 2. ⛔ FIGURE_MANIFEST 区块必须存在 (下游 paper-figure / paper-figure 按它对账)
...
if grep -q '<!-- BEGIN FIGURE_MANIFEST -->' PROBLEM_ANALYSIS.md 2>/dev/null \
    ...
    FIG_COUNT=$(sed -n "${START},${END}p" PROBLEM_ANALYSIS.md \
        | grep -cE '^[[:space:]]*-[[:space:]]+(fig_[a-zA-Z0-9_]+|tikz_[a-zA-Z0-9_]+)')
    if [ "$FIG_COUNT" -lt 3 ]; then
        echo "❌ FIGURE_MANIFEST 区块只列了 $FIG_COUNT 张图, 太少 (竞赛通常 ≥ 3 张, 至少含技术路线图 + 求解流程图)"
        PASS=false
    else
        echo "✅ FIGURE_MANIFEST ($FIG_COUNT 张图)"
    fi
else
...
**写完上面三类图清单（数据图 + DrawIO + TikZ + GPT Image）后，在 PROBLEM_ANALYSIS.md **最后**追加一个机器可读的清单区块，下游 paper-figure / paper-figure 会按此清单逐条对账。少一张就报错。**
