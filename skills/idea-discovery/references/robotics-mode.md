...
```
/literature-review → /idea-creator (robotics framing) → /novelty-check → /research-review
...
> Override inline, e.g. `/idea-discovery "bimanual manipulation" — only sim ideas, no real robot` or `/idea-discovery "drone navigation" — focus on CoRL/RSS, 2 pilot ideas max`
...
```
/literature-review "$ARGUMENTS — focus venues: CoRL, RSS, ICRA, IROS, RA-L, TRO, Science Robotics"
```
...
**Direction**: $ARGUMENTS
**Date**: [today]
**Pipeline**: literature-review → idea-creator (robotics framing) → novelty-check → research-review
...
```
/idea-discovery "direction"   ← you are here
implement sim-first pilot
