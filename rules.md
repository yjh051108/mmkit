...
> 本文件供 Trae IDE 读取。Trae 应将此文件作为项目规则加载。
...
- Trae 可通过 Bash 工具调用 `orchestrator/mm_flow.py`
- 子 skill 分发可用 Trae 的 `Task` 工具启动 subagent
- 审查环节 agent 自己读产物做 review，或调 `orchestrator/reviewer.py`
- 不需要任何 Trae 特定配置，只需能读 SKILL.md 和调 Bash 即可
